...
- **双栏工作区**：左侧集中连接、时间和输出配置，右侧展示点位列表、运行预览与日志，
  减少纵向滚动；顶部工具栏提供**新建任务、保存预设、加载预设、历史记录**
- 图形界面配置 **地址 / 端口 / 用户名 / 密码 / 数据库**
- 顶部彩色**连接状态标签**清晰区分未连接、连接中、已连接和连接失败
- 输入项提供**悬停说明和即时校验**；无效内容会标红并给出原因，修正后即可恢复
- 配置 **起始 / 终止时间**、**分段大小（小时）**、**UTC 偏移**、取值字段、点位标签列
- 可编辑的**点位表格**：固定表头、斑马纹、名称/类型/备注搜索、已选数量提示，
  并支持增删改、启用/停用、从剪贴板批量粘贴（`点位,类型,备注`）
...
- 一键**测试连接**（结果通过顶部状态标签和日志即时反馈，不阻塞后续操作）
...
- 点位列表支持**一键清空**
- 后台线程拉取，进度区实时显示**当前分段、速度、已耗时和预计剩余时间**，可随时**取消**
- **运行日志默认折叠**，开始拉取或发生错误时自动展开，也可随时手动展开、折叠或清空
- 拉取完成后显示**非阻塞通知**，可直接**打开文件、打开目录或再次拉取**
- 自动保存最近的拉取历史（不记录密码），可从顶部「历史记录」查看状态、耗时、行数和输出路径
- 自动按时间分段查询、去重、排序、合并，导出为 **CSV（UTC+本地时间还原）**
- **CSV 点位备注**：有备注的点位以 `点位名（备注）` 作为导出列名，无备注点位保持原名；数值数据和单行表头结构不变
- 简洁的原生应用图标，同时用于 Windows 任务栏和程序窗口标题栏
- **保存 / 载入** JSON 配置预设
